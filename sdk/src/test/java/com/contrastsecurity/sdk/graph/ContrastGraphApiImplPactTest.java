...
final class ContrastGraphApiImplPactTest {
    ...
    private ContrastGraphApiImpl client(final MockServer server) {
        ContrastSDK sdk = new ContrastSDK.Builder("user", "serviceKey", "apiKey")
        ...
        RequestResponsePact pact(final PactDslWithProvider builder) {
            return builder
                    ...
                    .body(newJsonBody(body -> {
                        body.array("nodes", nodes -> nodes.object(node -> {
                        ...
                        body.array("graphAssets", assets -> assets.object(asset -> {
        ...
        RequestResponsePact pact(final PactDslWithProvider builder) {
            return builder
                    .given("incident exists",
                            Collections.singletonMap("incidentId", INCIDENT_ID))
                    .uponReceiving("get incident graph")
                    .method("GET")
                    .path("/Contrast/api/v2/organizations/" + ORG_ID
                            + "/contrast-graph/incidents/" + INCIDENT_ID)
                    .willRespondWith()
                    .status(200)
                    .body(newJsonBody(body -> {
                        body.array("nodes", nodes -> nodes.object(node -> {
    ...
    final class GetFacets {
        ...
        RequestResponsePact pact(final PactDslWithProvider builder) {
            return builder
                    ...
                    .method("POST")
                    .path("/Contrast/api/v2/organizations/" + ORG_ID
                            + "/contrast-graph/facets/nodeTypes")
                    .willRespondWith()
                    .status(200)
                    .body(newJsonBody(body ->
                            body.array("nodeTypes", facets -> facets.object(facet -> {
        ...
        void getFacets(final MockServer server) throws IOException {
            FacetsResponse response =
                    client(server).getFacets(ORG_ID, "nodeTypes", new RequestFilters());
    ...
    final class GetApplicationLibraries {
        ...
        RequestResponsePact pact(final PactDslWithProvider builder) {
            return builder
                    ...
                    .method("POST")
                    .path("/Contrast/api/v2/organizations/" + ORG_ID
                            + "/contrast-graph/applications/" + APP_ID + "/libraries")
                    .query("agentReportingInstanceId=" + AGENT_INSTANCE_ID)
                    .willRespondWith()
                    .status(200)
                    .body(newJsonBody(body -> {
                        body.array("dependencies", deps -> deps.object(dep -> {
        ...
        void getApplicationLibraries(final MockServer server) throws IOException {
            ApplicationLibrariesResponse response =
                    client(server).getApplicationLibraries(
                            ORG_ID, APP_ID, AGENT_INSTANCE_ID, null);
    ...
    final class GetApplicationLibraryDetails {
        ...
        RequestResponsePact pact(final PactDslWithProvider builder) {
            return builder
                    ...
                    .method("GET")
                    .path("/Contrast/api/v2/organizations/" + ORG_ID
                            + "/contrast-graph/applications/" + APP_ID
                            + "/libraries/" + LIBRARY_HASH)
                    .willRespondWith()
                    .status(200)
                    .body(newJsonBody(body -> {
